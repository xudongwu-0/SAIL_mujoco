...
class RewardModel:
    ...
    def train_reward(self, critic):
        ...
        for epoch in range(num_epochs):
            ...
            for member in range(self.de):
                ...
                # -----------------------------
                # Q terms (optional; only when critic is available)
                sa_t_1_temp = torch.as_tensor(sa_t_1, dtype=torch.float32).to(device)
                sa_t_2_temp = torch.as_tensor(sa_t_2, dtype=torch.float32).to(device)

                if critic is None:
                    # TRPO / on-policy mode: no Q critic. Disable Q shaping safely.
                    bsz = sa_t_1_temp.shape[0]
                    T = sa_t_1_temp.shape[1]
                    Q1 = torch.zeros((bsz, T), device=device)
                    Q2 = torch.zeros_like(Q1)
                    Q1_2 = torch.zeros((bsz, T), device=device)
                    Q2_2 = torch.zeros_like(Q1_2)
                    q_reg = 0.0
                else:
                    Q1, Q2 = critic.forward_v(obs_action=sa_t_1_temp)
                    Q1_2, Q2_2 = critic.forward_v(obs_action=sa_t_2_temp)
                    # Keep your original behavior: only regularize with Q1 (not Q1_2)
                    q_reg = (lambda1 * Q1.sum(axis=1).mean())

                # -----------------------------
                # reward logits
                r_hat1 = self.r_hat_member(sa_t_1, member=member)  # + lambda1 * Q1
                r_hat2 = self.r_hat_member(sa_t_2, member=member)  # + lambda1 * Q1_2
                r_hat1 = r_hat1.sum(axis=1)
                r_hat2 = r_hat2.sum(axis=1)
                r_hat = torch.cat([r_hat1, r_hat2], axis=-1)

                # compute loss
                curr_loss = self.CEloss(r_hat, labels) + q_reg
                loss += curr_loss
    ...
    def train_soft_reward(self, critic):
        ensemble_losses = [[] for _ in range(self.de)]
        ensemble_acc = np.array([0 for _ in range(self.de)])
        ...
        for epoch in range(num_epochs):
            ...
            for member in range(self.de):
                ...
                curr_loss = self.softXEnt_loss(r_hat, target_onehot) + lambda1 * Q1.sum(axis=1).mean()# + lambda1 * Q1_2.sum(axis=1).mean()
                loss += curr_loss
                ensemble_losses[member].append(curr_loss.item())
