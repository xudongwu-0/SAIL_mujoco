...
AGENT_TRAIN_FORMAT = {
    ...
    ('batch_reward', 'BR', 'float'),
    ('trpo_obj', 'TOBJ', 'float'),
    ('trpo_expected_improve', 'EIMP', 'float'),
    ('trpo_linesearch_success', 'LSS', 'float'),
    ('trpo_kl', 'KL', 'float'),
    ],
}
...
class MetersGroup(object):
    ...
    def _prime_meters(self):
        ...
        for key, meter in self._meters.items():
            if key.startswith('train'):
                ...
                key = key[len('eval') + 1:]
            elif key.startswith('trpo'):
                key = key[len('trpo') + 1:]
            else:
                # unknown prefix, keep as-is (or skip)
                pass
            key = key.replace('/', '_')
    ...
    def _dump_to_csv(self, data):
        if self._csv_writer is None:
            self._csv_writer = csv.DictWriter(self._csv_file, fieldnames=sorted(data.keys()), restval=0.0, extrasaction='ignore')
...
class Logger(object):
        ...
        if save_tb:
            ...
            if os.path.exists(tb_dir):
                try:
                    ...
                    pass
            self._sw = None#SummaryWriter(tb_dir)
        else:
            self._sw = None
    ...
    def log(self, key, value, step, n=1, log_frequency=1):
        ...
        assert key.startswith('train') or key.startswith('eval') or key.startswith('trpo')
        if type(value) == torch.Tensor:
            value = value.item()
        self._try_sw_log(key, value / n, step)
        if key.startswith('train') or key.startswith('trpo'):
            mg = self._train_mg
        else:
            mg = self._eval_mg
        mg.log(key, value, n)
